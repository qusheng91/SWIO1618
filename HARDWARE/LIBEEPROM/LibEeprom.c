...
//读一个字节函数
s8 eeprom_read_byte(u16 addr, u8 *data)
{
    int8_t res = -1;
...
static void eeprom_write_disable(void)
{
    EEPROM_CS_ENABLE;
    delay_us(DE_GAP);
    eeprom_read_write_byte(EEPROM_WRDI_CMD, NULL);
    EEPROM_CS_DISABLE;
    delay_us(DE_GAP);

}

s8 eeprom_write_byte(u16 addr, u8 data)
{
    int8_t res = -1;
...
s8 eeprom_write_page(u16 addr, u8 *data, u16 size)
{
    int8_t res = 1;
    uint16_t i;
